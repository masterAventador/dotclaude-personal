...
**Worktree 存放位置:** 为项目创建 Git worktree 时，必须统一创建在当前项目根目录的 `wt/` 文件夹下，完整路径格式为 `<项目根目录>/wt/<worktree名称>`。禁止创建到 `/tmp`、`/private/tmp`、项目同级目录或其他位置。创建前应优先把 `wt/` 加入仓库本地的 `.git/info/exclude`，避免污染共享 `.gitignore`；创建完成后必须向用户报告 worktree 的完整绝对路径，方便查找和集中清理。

**提交时机:** 每个 Task 的实现、适用测试、真实边界验证、资源清理和任务台账全部通过后，必须主动提交当前 Task 的改动，不需要等待用户再次下令。任务尚未达到完成门禁时不得为了形成提交而提前提交。

**提交后自动推送:** 每个 Task 提交后默认立即推送当前分支。只有用户明确说“只提交不推送”或“暂不提交”时才暂停对应动作。仓库没有远端或缺少写权限时，先保留本地提交并报告阻塞；除非用户已授权，否则不擅自创建新的远端仓库。
...
**核心规则:** 无论开发的是桌面 App、Flutter、Tauri、H5/Web、移动端、服务端或其他技术栈，自动化测试和本地验收只要支持后台运行就必须放到后台运行，只要支持无头（headless）模式就必须使用无头模式；不得无故弹出浏览器、模拟器、桌面窗口、安装界面或其他可见 UI 打断用户正常使用电脑。只有用户明确要求观察界面、进行人工交互或底层工具确实不支持静默运行时，才允许显示窗口，并须提前说明影响。测试结束、失败或中断后，必须及时关闭本轮启动的浏览器、模拟器、App、驱动及辅助进程，避免持续占用 CPU 和内存。
...
**核心规则:** 开发过程中如果本机缺少某个工具，并且该工具当前任务需要、后续开发也大概率会持续使用，直接使用 Homebrew 全局安装，不要为了绕过安装而改用功能不匹配、操作不顺手或难以维护的替代工具。

执行前只做必要检查：

- 确认本机没有已经可用的同类工具或另一个 Homebrew 版本；
- 确认不会与项目锁定版本、系统自带工具或现有 PATH 产生冲突；
- 优先安装官方维护或项目约定的 formula/cask；
