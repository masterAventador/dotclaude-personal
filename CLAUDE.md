...
**Worktree 存放位置:** 为项目创建 Git worktree 时，必须统一创建在当前项目根目录的 `wt/` 文件夹下，完整路径格式为 `<项目根目录>/wt/<worktree名称>`。禁止创建到 `/tmp`、`/private/tmp`、项目同级目录或其他位置。创建前应优先把 `wt/` 加入仓库本地的 `.git/info/exclude`，避免污染共享 `.gitignore`，方便集中查找和清理。
